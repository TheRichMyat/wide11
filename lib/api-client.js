...
export const api = {
  ...
  // Account (admin self-service: change own email / password)
  getAccount: () => apiFetch('/api/account'),
  updateAccount: (data) => apiFetch('/api/account', { method: 'PUT', body: JSON.stringify(data) }),

  // News (public list returns is_published only; admin list adds ?all=true)
  getNews: () => apiFetch('/api/news'),
  getAllNews: () => apiFetch('/api/news?all=true'),
  getNewsItem: (id) => apiFetch(`/api/news/${id}`),
  createNews: (data) => apiFetch('/api/news', { method: 'POST', body: JSON.stringify(data) }),
  updateNews: (id, data) => apiFetch(`/api/news/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteNews: (id) => apiFetch(`/api/news/${id}`, { method: 'DELETE' }),
